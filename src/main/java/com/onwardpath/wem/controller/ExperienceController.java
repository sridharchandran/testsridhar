...
public class ExperienceController {
	   ...
	   expseg.saveExperience(newcontentexp);
		...
		int experience_id = newcontentexp.getId();
		 ...
		 String experienceDetails = request.getParameter("experienceDetails");	            		  	            		 
		  ...
		  session.setAttribute("message", "Experience <b>"+name+"</b> saved. You can now configure the pages for this experience. #n="+name+"#e="+experience_id+"#o="+newcontentexp.getOrgId()+"#t="+type); 
	...
	public String submitconfig(ModelMap mp,HttpSession session,HttpServletRequest request) throws JsonMappingException, JsonProcessingException, ParseException {
        ...
        if(request.getParameter("status")!=null && request.getParameter("status") !="") {
        	...
        	expRepo.updateexperience(newexp.getTimezoneId(), newexp.getScheduleStart(), newexp.getScheduleEnd(), newexp.getStatus(),experience_id);
